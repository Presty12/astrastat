...
client.on ( "voiceChannelJoin" , ( member , channel ) => {
    ...
    if (!member.roles.cache.has ("787988311166681089")) return
    const json = {
        "channel" : channel.id ,
...
client.on ( "message" , async message => {
    ...
    if(!['787988311166681089'].some(role => message.member.roles.cache.get(role)) && !message.member.hasPermission('ADMINISTRATOR')) return message.react('❌')
    var totall = (
                     await db.fetch (
...
client.on ( "message" , async msg => {
    if ( msg.content.startsWith ( ".stat" ) || msg.content.startsWith ( ".me" ) ) {
        ...
        if(!['787988311166681089'].some(role => msg.member.roles.cache.get(role)) && !msg.member.hasPermission('ADMINISTRATOR')) return msg.react('❌')
        let st = member.voice
        ...
        const embed = new Discord.MessageEmbed ()
            ...
            .setFooter('Kullanıcının verileri')
            .setColor ( "RANDOM" )
...
client.on ( "message" , async msj => {
    ...
    if(!['787988311166681089'].some(role => msj.member.roles.cache.get(role)) && !msj.member.hasPermission('ADMINISTRATOR')) return msj.react('❌')
    ...
    const toplam = new Discord.MessageEmbed ()
        ...
        .setDescription ( ` Roma'ya ait top ses&mesaj aktifliği listesi
...
client.on("ready", async () => {
  client.user.setPresence({ activity: { name: "Gods are loves 1001"}, status: "dnd" }); //dnd rahatsı etmeyın, online çevirimiçi, idle boşta
});
